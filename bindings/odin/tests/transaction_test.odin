...
count_rows :: proc(conn: turso.Connection) -> i64 {
	n, _, _ := turso.db_scalar_i64(conn, "SELECT COUNT(*) FROM t")
	return n
}

test_db_with_transaction_commit :: proc() {
	t := txn_setup("txn_commit")
	defer test_db_close(&t)

	e, ok := turso.db_with_transaction(t.conn, proc(c: turso.Connection) -> bool {
		_, _, _ = turso.db_exec(c, "INSERT INTO t(v) VALUES (1)")
		_, _, _ = turso.db_exec(c, "INSERT INTO t(v) VALUES (2)")
		return true
	})
	expect_no_err(e, ok, "db_with_transaction commit path")
	expect_eq(count_rows(t.conn), i64(2), "rows must persist after commit")
}

test_db_with_transaction_rollback :: proc() {
	...
	expect_no_err(e, ok, "db_with_transaction rollback path")
	expect_eq(count_rows(t.conn), i64(0), "rollback must discard the inserts")
}

test_db_with_transaction_manual_commands :: proc() {
	t := txn_setup("txn_manual")
	defer test_db_close(&t)

	e1, ok1 := turso.db_begin(t.conn)
	expect_no_err(e1, ok1, "db_begin")
	_, _, _ = turso.db_exec(t.conn, "INSERT INTO t(v) VALUES (10)")
	e2, ok2 := turso.db_rollback(t.conn)
	expect_no_err(e2, ok2, "db_rollback")
	expect_eq(count_rows(t.conn), i64(0), "manual rollback discards inserts")

	e3, ok3 := turso.db_begin(t.conn)
	expect_no_err(e3, ok3, "db_begin again")
	_, _, _ = turso.db_exec(t.conn, "INSERT INTO t(v) VALUES (20)")
	e4, ok4 := turso.db_commit(t.conn)
	expect_no_err(e4, ok4, "db_commit")
	expect_eq(count_rows(t.conn), i64(1), "manual commit persists insert")
}

test_db_with_savepoint_release :: proc() {
	t := txn_setup("savepoint_release")
	defer test_db_close(&t)

	_, _, _ = turso.db_exec(t.conn, "INSERT INTO t(v) VALUES (1)")
	e, ok := turso.db_with_savepoint(t.conn, "inner", proc(c: turso.Connection) -> bool {
		_, _, _ = turso.db_exec(c, "INSERT INTO t(v) VALUES (2)")
		return true
	})
	expect_no_err(e, ok, "db_with_savepoint release path")
	expect_eq(count_rows(t.conn), i64(2), "savepoint release must keep inner insert")
}

test_db_with_savepoint_rollback :: proc() {
	...
	e, ok := turso.db_with_savepoint(t.conn, "inner", proc(c: turso.Connection) -> bool {
		_, _, _ = turso.db_exec(c, "INSERT INTO t(v) VALUES (2)")
		_, _, _ = turso.db_exec(c, "INSERT INTO t(v) VALUES (3)")
		return false
	})
	expect_no_err(e, ok, "db_with_savepoint rollback path")
...
test_db_with_savepoint_nested :: proc() {
	t := txn_setup("savepoint_nested")
	defer test_db_close(&t)

	e, ok := turso.db_with_savepoint(t.conn, "outer", proc(c: turso.Connection) -> bool {
		_, _, _ = turso.db_exec(c, "INSERT INTO t(v) VALUES (1)")
		_, inner_ok := turso.db_with_savepoint(c, "inner", proc(c2: turso.Connection) -> bool {
			_, _, _ = turso.db_exec(c2, "INSERT INTO t(v) VALUES (2)")
			return false
		})
...
// transaction.
test_db_with_transaction_body_failure_propagates :: proc() {
	t := txn_setup("txn_body_fail")
	defer test_db_close(&t)

	e, ok := turso.db_with_transaction(t.conn, proc(c: turso.Connection) -> bool {
		_, _, _ = turso.db_exec(c, "INSERT INTO t(v) VALUES (1)")
		_, ie, iok := turso.db_exec(c, "INSERT INTO nonexistent_table VALUES (1)")
	...
	_, _, _ = turso.db_exec(t.conn, "INSERT INTO t(v) VALUES (99)")
